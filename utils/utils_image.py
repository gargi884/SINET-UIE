...
def imread_uint1(path, n_channels=1,patch_size=256):
    #  input: path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
        img = cv2.imread(path, 0)  # cv2.IMREAD_GRAYSCALE
    ...
    H, W, _ = img.shape
    if H<patch_size:
        img = cv2.resize(img, (patch_size,W), cv2.INTER_CUBIC)
        if W<patch_size:
            img = cv2.resize(img, (patch_size,patch_size), cv2.INTER_CUBIC)
            # print(img.shape)
    if n_channels == 1:
        img = np.expand_dims(img, axis=2)  # HxWx1
    return img
